...
export default {
    id: 'match',
    data: [], selectedBlocks: [], matchedCount: 0, index: 0, currentRoundItems: [],
    ...
    init(data, settings) {
        this.data = shuffle(data); // Trộn 1 lần
        this.index = 0; // Bắt đầu từ từ vựng số 0

        document.getElementById('match-total').textContent = this.data.length;

        document.getElementById('match-restart').onclick = () => {
            this.index += this.currentRoundItems.length; // Cộng dồn số từ đã chơi
            this.startRound();
        };
        this.startRound();
    },
    ...
    startRound() {
        ...
        // Lấy 5 từ kế tiếp (hoặc ít hơn nếu danh sách sắp hết)
    ...
    createCardElement(blk) {
        ...
        if (blk.type === 'vi') {
            div.classList.replace('font-label-lg', 'font-body-sm');
        ...
        if (this.selectedBlocks.length === 2) {
            ...
            if (first.wordId === second.wordId) {
                setTimeout(() => {
                    ...
                    // Nếu số thẻ nối đúng bằng với số thẻ của vòng này (Thường là 5)
                    if (this.matchedCount === this.currentRoundItems.length) {
                        ...
                        // Nếu là màn cuối cùng
                        if (this.index + this.matchedCount >= this.data.length) {
                            document.getElementById('match-restart').textContent = "Hoàn thành bài tập 🎉";
                        }
